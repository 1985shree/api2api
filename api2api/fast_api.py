...
def predict_picnic(date_from,
    ...
    start_date_dt = datetime.strptime(date_from, "%Y-%m-%d")
    end_date_dt = datetime.strptime(date_to, "%Y-%m-%d")
    ...
    for day in day_list:
        places = ['pankow, berlin', 'spandau, berlin']
        for loc in places:

            return {'location' : type(loc)}
